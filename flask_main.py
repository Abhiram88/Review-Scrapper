...
def scrapper():
    ...
    #return 'hello'
    return render_template('display.html', result=reviews_dataset)
...
def scrapper():
    ...
    reviews_dataset = main_prog(product_info, search_query)
    mongo_dataset = reviews_dataset

    if product_info:
        print(product_info)
        review_db = MongoClient()
        conn = review_db['review_scrapper']
        reviews = conn.reviews_collection

        #MongoDB doesn't accept '. or $' in keys.

       # print(mongo_dataset['comments'].keys())
        #print(len(mongo_dataset['comments'].keys()))
        test = list(mongo_dataset['comments'].keys())
        print(test)
        for key in test:
                print(key)
                new_key = key.replace(".", "_")
                print(new_key)
                mongo_dataset['comments'][new_key] = mongo_dataset['comments'].pop(key)

        print("----------------------")
        print(mongo_dataset['comments'].keys())
        print("-------------------")

        reviews.insert_many([mongo_dataset])
    else:
...
def base_content(search_string):
    ...
    boxes_limit = len(bigboxes[:-2])
    #print(boxes_limit)

    for linc in bigboxes[2:boxes_limit]:
        divs = linc.div.div

        img = linc.find_all('a', title=True, href=True)
        for name in img:
...
#    print(product_links)
    ...
    '''models = page_beautified.find_all('div', {'class': 'bhgxx2 col-12-12'})
    links = page_beautified.find_all('div', {'class': ['_1UoZlX', '_3liAhj']})

    links_arr = []
    for item in models:
        for value in item:
            anchor = value.find_all('a')
            for content in anchor:
                products.append(content.get('title'))
                #print(content.get('title'))
                links_arr.append(content['href'])'''
